...
function Install-VimrcBase {
    ...
    Write-Info "生成 .vimrc..."
    # 不用 amix 的 bash 安装脚本写 .vimrc（它在 Windows Git Bash 上有路径/编码问题）
    # 直接在 PowerShell 中生成，内容与 amix 安装脚本完全一致
    # vim 中路径用正斜杠（Windows/Linux 通用）
    $vr = $vimRuntime -replace '\\', '/'
    $vimrcContent = @"
" DO NOT EDIT THIS FILE
" Add your own customizations in $vr/my_configs.vim

set runtimepath+=$vr

source $vr/vimrcs/basic.vim
source $vr/vimrcs/filetypes.vim
source $vr/vimrcs/plugins_config.vim
source $vr/vimrcs/extended.vim

try
  source $vr/my_configs.vim
catch
endtry
"@
